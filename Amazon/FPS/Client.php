...
class Amazon_FPS_Client implements Amazon_FPS_Interface
{
    ...
    private  $_config = array ( 'ServiceURL' =>'https://fps.sandbox.amazonaws.com', //('ServiceURL' => 'https://fps.amazonaws.com',
    ...
    private function _reportAnyErrors($responseBody, $status, Exception $e =  null)
    {
        ...
        if (!is_null($responseBody) && strpos($responseBody, '<') === 0) {
            ...
            if (preg_match('@<RequestId>(.*)</RequestId>.*<Error><Code>(.*)</Code><Message>(.*)</Message></Error>.*(<Error>)?@mi', $responseBody, $errorMatcherOne)) {
            ...
            } elseif (preg_match('@<Error><Code>(.*)</Code><Message>(.*)</Message></Error>.*(<Error>)?.*<RequestID>(.*)</RequestID>@mi', $responseBody, $errorMatcherTwo)) {
                ...
                $ex = new Amazon_FPS_Exception(array ('Message' => $message, 'StatusCode' => $status, 'ErrorCode' => $code,
                                                              'ErrorType' => 'Unknown', 'RequestId' => $requestId, 'XML' => $responseBody));
            } elseif (preg_match('@<Error><Type>(.*)</Type><Code>(.*)</Code><Message>(.*)</Message>.*</Error>.*(<Error>)?.*<RequestId>(.*)</RequestId>@mi', $responseBody, $errorMatcherThree)) {
            ...
            }  elseif($xml->Errors!=null && $xml->Errors->Error!=null) {
                if($xml->Errors->Error->Code!=null) {
                    $code = $xml->Errors->Error->Code;
                }
                if($xml->Errors->Error->Message!=null) {
                    $message = $xml->Errors->Error->Message;
                }
                if($xml->RequestID!=null) {
    ...
    private function _httpPost(array $parameters)
    {
        ...
        curl_setopt($curlHandle, CURLOPT_SSL_VERIFYPEER, false); //Todo fix!!
        ...
        if(curl_errno($curlHandle)!=0) {
            throw new Exception("Error sending payment: ". curl_errno($curlHandle));
        }
