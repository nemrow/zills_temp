...
function iColorShow(i,i2){
	...
	jQuery(".izzyColors a, #colorPreview").unbind().mouseover(function(){
		...
		$('.colorChoiceSample').css({"background":izzyColor}).trigger('change');
		if($('.colorChoiceSample').attr('sexytime')=='sex'){
			// do nothing
		}else{
			$('.colorChoiceSample').attr('sexytime','sex');
			$('.rockstarSil').animate({
				'height' : '-=8%'
			},600);
		};
...
this.izzyColor=function(){
	...
	jQuery("input.izzyColor").each(function(i){
		...
		.after('<a href="javascript:void(null)" id="icp_'+this.id+'" onclick="iColorShow(\''+this.id+'\',\'icp_'+this.id+'\')"><img class="colorSelectorCirlce" src="'+imageUrl+'" style="border:0;margin:4px 0 0 5px" align="absmiddle" ></a>')
